...
\hypersetup{
    ...
    pdftitle={GEXF 1.3draft Primer},    % title
    pdfauthor={Sebastien Heymann},     % author
    pdfsubject={GEXF 1.3draft Primer},   % subject of the document
...
\paragraph{}
The second line contains the root-element element of a GEXF document: the gexf element. The gexf element, like all other GEXF elements, belongs to the namespace \begin{footnotesize}http://www.gexf.net/1.3draft\end{footnotesize}. For this reason we define this namespace as the default namespace in the document by adding the XML Attribute \begin{footnotesize}xmlns="http://www.gexf.net/1.3draft"\end{footnotesize} to it. The two other XML Attributes are needed to specify the XML Schema for this document. In our example we use the standard schema for GEXF documents located on the gexf.net server. The first attribute, \begin{footnotesize}xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"\end{footnotesize}, defines xsi as the XML Schema namespace. The second attribute, \begin{footnotesize}xsi:schemaLocation ="http://www.gexf.net/1.3draft http://www.gexf.net/1.3draft/gexf.xsd"\end{footnotesize}, defines the XML Schema location for all elements in the GEXF namespace.
...
\paragraph{}
GEXF document is specified in the \href{http://relaxng.org/compact-tutorial-20030326.html}{RelaxNG Compact} file \href{http://www.gexf.net/1.3draft/gexf.rnc}{gexf.rc}. Header is ruled by the following declaration :
...
The optional XML-attribute \begin{footnotesize}mode\end{footnotesize} set the kind of network: static, dynamic or slice for a collection of files encoding a dynamic network. The last two provide time support (see  the section \ref{dynamics} on Dynamics). Static mode is assumed by default.
...
Each edge can have an optional XML-attribute \begin{footnotesize}label\end{footnotesize}, which is a string.

The optional XML-attribute \begin{footnotesize}type\end{footnotesize} declares if the edge is \textit{directed}, \textit{undirected} or \textit{mutual} (directed \textit{from source to target and from target to source}). If the direction is not explicitly defined, the default direction is applied to this edge as defined in the enclosing graph.

In addition the optional XML-attribute \begin{footnotesize}kind\end{footnotesize} can be used to further scecify edges in e.g. multi-graphs. 

% we need an example here preferably of a multi-graph with self-loops.
...
\paragraph{}
A bunch of data can be stored within attributes. The concept is the same as table data or SQL. An attribute has a title/name and a value. Attribute's name/title must be declared for the whole graph. It could be for instance "degree"Äù, "valid" or "url"Äù. Besides the name of the attribute another column  contains the type.
...
%
%GEXF format allows creating hierarchical graph structure essentially for representing clustering. We model both a tree structure of ancestors and descendents, and a flat graph of nodes bound by edges.
%
%\begin{figure}[!ht]
%  \begin{center}
...
And of course the \begin{footnotesize}end\end{footnotesize} value must be later than the \begin{footnotesize}start\end{footnotesize} value. 
\paragraph{}
...
Using the visualization module must be declared by adding the XML Attribute \begin{footnotesize}xmlns:viz="http://www.gexf.net/1.3draft/viz"\end{footnotesize} to the document namespaces. The \begin{footnotesize}xsi:schemaLocation\end{footnotesize} attribute includes the XML-Schema declaration of the VIZ module. The RelaxNG Compact specification is available in \href{http://www.gexf.net/1.3draft/viz.rnc}{viz.rnc}, and independent XSD declaration in \href{http://www.gexf.net/1.3draft/viz.xsd}{viz.xsd}.
...
Colors are either defined by the \href{http://en.wikipedia.org/wiki/RGBA}{RGBA color model} or by an html-like hex syntax. Each XML-attribute value \begin{footnotesize}r\end{footnotesize}, \begin{footnotesize}g\end{footnotesize} or \begin{footnotesize}b\end{footnotesize} is hence an integer from 0 to 255. Alternatively color can be defined as hex value \begin{footnotesize}hex\end{footnotesize} with or without the initial "\#" symbol.  In both cases an optional alpha value \begin{footnotesize}a\end{footnotesize} as a float from 0.0 to 1.0 can be added. 

\lstset{ style=gexf }
\begin{lstlisting}[caption={VIZ Color Declaration RGB},label=vizcolor]
<viz:color r="239" g="173" b="66" a="0.5"/>
\end{lstlisting}

\lstset{ style=gexf }
\begin{lstlisting}[caption={VIZ Color Declaration HEX},label=vizcolor]
...
\begin{itemize}
\item 29 December 2015 -- Changes to 1.3
